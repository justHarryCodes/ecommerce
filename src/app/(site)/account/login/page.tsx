  ...
  return (
    <div className="max-w-md mx-auto px-4 py-16">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="block text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Password</label>
            <Link href="/auth/forgot-password" className="text-xs font-medium hover:underline" style={{ color: "var(--accent)" }}>
              Forgot password?
            </Link>
          </div>
      ...
      <p className="text-center text-sm mt-6" style={{ color: "var(--text-secondary)" }}>
        New here?{" "}
        <Link href={`/account/signup?next=${encodeURIComponent(next)}`} className="font-semibold hover:underline" style={{ color: "var(--accent)" }}>
          Create an account
        </Link>
      </p>
    </div>
