...
export default function CartDrawer({ isOpen, onClose, company }: CartDrawerProps) {
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ background: "var(--bg)", boxShadow: "-8px 0 40px rgba(0,0,0,0.15)" }}
      >
        {/* Header */}
        <div
        ...
        >
          <div className="flex items-center gap-2.5">
            <ShoppingBag className="h-5 w-5" style={{ color: "var(--text-primary)" }} />
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center py-16">
              ...
              >
                <ShoppingBag className="h-7 w-7" style={{ color: "var(--text-muted)" }} />
              </div>
              <div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button
                      onClick={() => updateQuantity(item.product_id, item.quantity + 1)}
                      className="h-6 w-6 rounded-lg flex items-center justify-center text-white transition-colors"
                      style={{ background: "var(--accent)" }}
        ...
        {items.length > 0 && (
          <div className="p-4 space-y-3 border-t" style={{ borderColor: "var(--border)" }}>
            ...
            <Link
              href="/checkout"
              onClick={onClose}
              className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl font-bold text-sm text-white transition-all hover:opacity-90 active:scale-[0.98] shadow-[0_4px_14px_rgba(0,0,0,0.15)]"
              style={{ background: "var(--accent)" }}
            >
            ...
            {whatsapp && (() => {
              const itemLines = items
                .map((it) => `• ${it.name} × ${it.quantity} — ${formatCurrency(it.price * it.quantity)}`)
                .join("\n");
              const msg = [
